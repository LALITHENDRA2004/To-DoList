...
class ThemeSwitcher {
  ...
  toggleTheme() {
    this.currentTheme = this.currentTheme === 'dark' ? 'cupcake' : 'dark';
    this.setTheme(this.currentTheme);
    this.updateIcon();
  ...
  updateIcon() {
    if (this.currentTheme === 'dark') {
      this.themeIcon.className = 'bx bxs-sun bx-sm theme-icon';
    } else {
      this.themeIcon.className = 'bx bxs-moon bx-sm theme-icon';
    }
  }
...
// Global function for filter functionality (called from HTML onclick)
function filterTodos(status) {
  uiManager.handleFilterTodos(status);
}
...
const themeSwitcher = new ThemeSwitcher(html);